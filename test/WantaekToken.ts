...
describe('WantaekToken', function () {
  ...
  it('deployer has the default admin role', async function () {
    ...
    expect(
      await wantaekToken.getRoleMemberCount(DEFAULT_ADMIN_ROLE),
    ).to.be.equal(BigNumber.from('1'));
    expect(await wantaekToken.getRoleMember(DEFAULT_ADMIN_ROLE, 0)).to.equal(
      deployer.address,
    );
  });
  ...
  it('deployer has the minter role', async function () {
    ...
    expect(await wantaekToken.getRoleMemberCount(MINTER_ROLE)).to.be.equal(
      BigNumber.from('1'),
    );
    expect(await wantaekToken.getRoleMember(MINTER_ROLE, 0)).to.equal(
      deployer.address,
    );
  });
  ...
  it('deployer has the pauser role', async function () {
    ...
    expect(await wantaekToken.getRoleMemberCount(PAUSER_ROLE)).to.be.equal(
      BigNumber.from('1'),
    );
    expect(await wantaekToken.getRoleMember(PAUSER_ROLE, 0)).to.equal(
      deployer.address,
    );
  });
  ...
  it('minter and pauser role admin is the default admin', async function () {
    ...
    expect(await wantaekToken.getRoleAdmin(MINTER_ROLE)).to.equal(
      DEFAULT_ADMIN_ROLE,
    );
    expect(await wantaekToken.getRoleAdmin(PAUSER_ROLE)).to.equal(
      DEFAULT_ADMIN_ROLE,
    );
  });
  ...
  describe('pausing', function () {
    ...
    it('cannot mint while paused', async function () {
      ...
      await expect(wantaekToken.mint(other.address, amount)).to.be.revertedWith(
    ...
    it('other accounts cannot pause', async function () {
      ...
      await expect(wantaekToken.connect(other).pause()).to.be.revertedWith(
    ...
    it('other accounts cannot unpause', async function () {
      ...
      await expect(wantaekToken.connect(other).unpause()).to.be.revertedWith(
  ...
  describe('burning', function () {
    it('holders can burn their tokens', async function () {
      ...
      await expect(receipt)
        .to.emit(wantaekToken, 'Transfer')
        .withArgs(other.address, zeroAddress, amount.sub(1));

      expect(await wantaekToken.balanceOf(other.address)).to.be.equal(
        BigNumber.from('1'),
      );
    });
  });
